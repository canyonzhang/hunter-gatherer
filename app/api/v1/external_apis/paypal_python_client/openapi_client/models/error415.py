...
class Error415(BaseModel):
    ...
    def message_validate_enum(cls, value):
        ...
        if value not in set(['The server does not support the request payload's media type.']):
            raise ValueError("must be one of enum values ('The server does not support the request payload's media type.')")
        return value
