...
class ErrorLinkDescription(BaseModel):
    ...
    """ # noqa: E501
    href: Annotated[str, Field(min_length=0, strict=True, max_length=20000)] = Field(description="The complete target URL. To make the related call, combine the method with this [URI Template-formatted](https://tools.ietf.org/html/rfc6570) link. For pre-processing, include the `$`, `(`, and `)` characters. The `href` is the key HATEOAS component that links a completed call with a subsequent call.")
    rel: Annotated[str, Field(min_length=0, strict=True, max_length=100)] = Field(description="The [link relation type](https://tools.ietf.org/html/rfc5988#section-4), which serves as an ID for a link that unambiguously describes the semantics of the link. See [Link Relations](https://www.iana.org/assignments/link-relations/link-relations.xhtml).")
    method: Optional[Annotated[str, Field(min_length=3, strict=True, max_length=6)]] = Field(default=None, description="The HTTP method required to make the related call.")
    __properties: ClassVar[List[str]] = ["href", "rel", "method"]

    @field_validator('href')
    def href_validate_regular_expression(cls, value):
    ...
    @field_validator('rel')
    def rel_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^.*$", value):
            raise ValueError(r"must validate the regular expression /^.*$/")
        return value
